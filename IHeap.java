...
interface IHeap extends IBinTree {
  ...
  * @param elt the root of a Heap / BT
  * @return returns the number of occurences of the given elt
  */
 int numOccurences(int elt);

 /**
  * Determines if the given Heap has the same elements as the BT excluding occurences
  ...
  * false otherwise.
  */
 public boolean sameAs(IBinTree aBT, int elt);

 /**
  * Gets the element that is being removed then determines if the given Heap
  * has the same elements as the BT excluding occurences of the item being removed
  * @param aBT a BT
  * @return True if the the given Heap contains the same elements as the BT,
  * false otherwise.
  */
 public boolean sameAs(IBinTree aBT);
}
...
class DataHeap extends DataBT implements IHeap {
 ...
 public boolean sameAs(IBinTree aBT, int elt){
  if (this.data == elt){
   return left.sameAs(aBT, elt) && right.sameAs(aBT, elt);
  }
  return aBT.hasElt(this.data) && left.sameAs(aBT, elt) && right.sameAs(aBT, elt);
 }
